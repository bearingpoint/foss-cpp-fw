...
public:
	...
	public:
		...
		bool operator !=(iterator const& i) const {
			assert(&i.parent_ == &parent_ && "iterators must belong to the same vector");
			return i.pos_ != pos_;
		}
		bool operator <(iterator const& i) const {
			assert(&i.parent_ == &parent_ && "iterators must belong to the same vector");
	...
	/**
	 * Thread-safe - blocks insertions from all threads, moves all the contents
	 * into a vector (which is then returned) and clears this container.
	*/
	std::vector<C> removeContentsExclusive() {
		std::vector<C> out;
		getContents(out, true);
		return out;
	}
	...
	void clear() {
		extra_.clear();
		for (size_t i=0; i<std::min(capacity_, insertPtr_.load()); i++)
			array_[i].~C();
...
private:
	...
	std::atomic<int> insertionsRunning_ {0};

	template<class ref>
	size_t insert(ref&& r) {
		while (insertionsBlocked_.load(std::memory_order_acquire))
			std::this_thread::yield();
		insertionsRunning_.fetch_add(1, std::memory_order_release);
		...
		++size_;
		insertionsRunning_.fetch_sub(1, std::memory_order_release);
		return writeIndex;
	}

	void getContents(std::vector<C> &out, bool doClear = false) {
		insertionsBlocked_.store(true, std::memory_order_release);
		// wait if anyone is currently inserting:
		while (insertionsRunning_.load(std::memory_order_acquire)) {
			// nothing, just loop
		}
		if (doClear) {
			std::move(begin(), end(), std::back_inserter(out));
			clear();
		} else {
