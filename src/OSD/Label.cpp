...
Label::Label(std::string value, FlexCoordPair pos, float textSize, glm::vec3 color)
	: pos_(pos)
	, color_(color)
	, textSize_(textSize)
	, value_(value) {
...
void Label::draw(RenderContext const& ctx) {
	GLText::get()->print(value_, pos_.get(ctx.viewport()), textSize_, color_);
	if (drawFrame) {
		glm::vec2 rectSize = boxSize() + glm::vec2(5, 5);
		Shape2D::get()->drawRectangle(
				pos_.get(ctx.viewport()) + glm::vec2{5, 5},
				rectSize,
				color_);
	}
}
