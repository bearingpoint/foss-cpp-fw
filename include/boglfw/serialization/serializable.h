...
template<typename T> int getSerializationType(T* t);
...
private:
	...
	struct model_t : concept_t {
		...
		static int getTypeImpl(T1* t, bool dummyToUseMember) {
			return t->getSerializationType();
		}
		template<typename T1>
		static int getTypeImpl(T1* t, ...) {
